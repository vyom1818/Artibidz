...
// Fetching artworks based on category
if (isset($_GET['category'])) {
    $category = $_GET['category'];
    $query = "SELECT a.*, 
                 (SELECT ai.art_image FROM art_image AS ai WHERE ai.art_id = a.art_id LIMIT 1) AS art_image
          FROM art AS a 
          JOIN sub_category AS sc ON a.sub_cat_id = sc.sub_cat_id
          JOIN category AS c ON sc.cat_id = c.cat_id
          WHERE c.cat_name LIKE '$category' AND a.sale_or_auction = 'sale'";
...
<section class="product-section section-title spad">
    <h2 style="color:#141414; margin-bottom:10vh">
        <?php echo isset($_GET['category']) ? $_GET['category'] : "No category selected"; ?>
    </h2>
    <div class="container">
...
<!-- Pagination -->
<?php if (isset($_GET['category']) && isset($result) && $result->num_rows > 0): ?>
    <?php
    $totalPages = ceil($result->num_rows / 20); // Assuming 20 items per page
